...
$email=mysqli_real_escape_string($obj->link_db, $_POST['email']);
$phone=mysqli_real_escape_string($obj->link_db, $_POST['phone']);
$image=mysqli_real_escape_string($obj->link_db, $_POST['image']);
$query="insert into users(name, address, email, phone, image) values('".$name."', '".$address."', '".$email."', '".$phone."', 'img/".$image."')";
mysqli_query($obj->link_db, $query);
break;
}
